...
export default function OtpStep({ email , setStep} : OtpStepProps) {
    ...
    //hooks
    //mutation
    const { verifyOtp , isPending: isVerfingOtp , error: verfiyOtpError} = useVerifyOtp ();
...
const onsubmit : SubmitHandler<OtpStepField> = (values)=>{
  ...
  return <>
     ...
     <Form {...form} >
          <form onSubmit={form.handleSubmit(onsubmit)} className="space-y-12 space-x-12">
          ...
          <FormItem className="w-full">
            ...
            <FormControl >
             <InputOTP maxLength={6} {...field}>
              {Array.from({length: 6}, (_, i) => i).map(i => <InputOTPSlot key={i} index={i} />)}
            </InputOTP>
            </FormControl>
            <FormMessage />
          </FormItem>
        ...
        <ResendOtp email={email}/>
            ...
            disabled={!form.formState.isValid && form.formState.isSubmitted} type="submit" className="mt-6 w-full bg-maroon-700" >
                {t('otp.verfiy')}
            </Button>
            </form>
        </Form>
...
function ResendOtp ({email}:{email : string | null}){
 //translation
    const t = useTranslations();
    //hooks
    const [otpCooldown, setValue, removeValue] = useLocalStorage(OTP_COOLDOWN_KEY, new Date(Date.now() + OTP_COOLDOWN_TIME).toISOString());
    ...
    const [countdown, setCountdown] = useState<number>(() => {
      ...
      const remainingTime = Math.floor((cooldown.getTime() - new Date().getTime()) / 1000);
